# WAIC 2026 展后精选

一份适合浏览和分享的 WAIC 2026 展后电子杂志，整理大会参展单位的新品、亮点与不可错过内容。35 个版面采用统一尺寸，并支持索引跳转、翻页、键盘和触控导航。

## 在线访问

[https://deronqi.github.io/waic-2026-aftershow-guide/](https://deronqi.github.io/waic-2026-aftershow-guide/)

## 内容范围

- 四条产业信号索引与 4 个独立解读页
- 十大不可错过索引与 10 个可独立分享的详情入口
- 25 项重点展品索引与 25 个独立详情页
- 六大赛道分类导航
- 每个详情页的原始发布链接
- 与 25 项展品一一对应的现场图或产品图
- 仓库内保留完整原始资料、整理文档和 ZIP，发布站点不展示过程资料

当前版本基于 33 份网页或模型卡原始资料和 11 张展商名录图片完成首轮整理，并非 1100 余家参展单位的全量数据库。

## 仓库结构

- `app/`：电子杂志网页源码
- `public/images/editorial/`：详情页现场图、官方资料图与图片来源说明
- `research/01_原始资料/`：33 份网页/模型卡快照和 11 张展商名录图片
- `research/02_摘要/`：首轮摘要、来源目录和图表说明
- `research/02_摘要/详情页配图来源.md`：现场图与官方资料图的原始链接及使用约定
- `research/WAIC2026_整理大纲.md`：完整整理大纲
- `research/WAIC2026_参展资料_首轮_20260721.zip`：完整资料压缩包

## 本地开发

需要 Node.js 22.13 或更高版本。

```bash
npm install
npm run dev
```

## 构建

验证 Sites / vinext 构建：

```bash
npm run build
```

生成 GitHub Pages 静态文件：

```bash
npm run build:pages
```

静态输出位于 `out/`。

## 内容说明

- “首发”不自动等同于产品成熟。
- 现场 Demo 不自动等同于跨场景泛化能力。
- “全球首个、唯一、领先”等描述按厂商口径处理，除非存在独立证据。
- 政府发布会使用“3000 余项技术产品”，部分展商名录报道使用“4000 余款展品”；站点采用政府口径并保留差异说明。

## 技术栈

- React 19
- Next.js 16
- vinext / Vite
- GitHub Pages 静态导出

## License

网站代码仅用于本项目展示。站点引用的厂商名称、产品信息及外部资料版权归各自权利人所有。
