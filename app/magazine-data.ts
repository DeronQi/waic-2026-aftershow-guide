...
export const tencentExhibit: Exhibit = {
  ...
  details: [
    ...
    {
      heading: "还要验证",
      body: "现场任务成功并不等于所有企业流程都能无人值守。真正应持续观察的是长任务成功率、失败恢复、企业权限与审计、敏感数据隔离、模型切换成本，以及交付物在真实工作流中还需多少人工返工。",
    },
  ],
  artwork: {
...
export const statusDefinitions = [
  ["首发", "在大会周期首次正式发布，需另查可用范围与交付时间。"],
  ["首秀", "第一次面向公众现场演示，不自动代表可购买或可部署。"],
  ["首次公开展出", "此前可能已发布，但本届第一次展示实物或完整方案。"],
  ["已上市", "存在明确购买、下载或 API 入口，仍需核对区域与配额。"],
  ["量产", "厂商披露进入批量生产或批量交付，应继续寻找订单和客户证据。"],
  ["试点", "已进入限定客户或限定场景验证，尚不能外推为规模部署。"],
] as const;
...
export const changes: ChangeStory[] = [
  {
    ...
    sections: [
      ...
      { heading: "验证问题", body: "优先追问已交付规模、真实模型负载、全系统功耗、连续运行时间、故障恢复和模型迁移成本。缺少这些证据时，“领先”只能保留为厂商口径。" },
    ],
  },
  {
    slug: "agents-deliver",
    ...
    sections: [
      ...
      { heading: "验证问题", body: "观察多次任务而非一次演示：成功率、平均接管次数、失败恢复、权限越界、调用成本和最终返工量，才是从“会做”走向“可靠交付”的关键证据。" },
    ],
  },
  {
    slug: "robots-work",
    ...
    sections: [
      ...
      { heading: "验证问题", body: "必须记录随机任务比例、样本数、连续运行时长、人工接管条件、电池与维护、节拍和回本周期。没有这些边界，精彩 Demo 不能被写成泛化或无人值守能力。" },
    ],
  },
  {
    slug: "personal-ai",
    ...
    sections: [
      ...
      { heading: "验证问题", body: "重点检查重量、续航、离线能力、录制指示、误触发纠正、跨应用权限、数据保存位置与删除机制。设备越贴近身体、在线越久，可信设计越重要。" },
    ],
  },
  {
    slug: "industry-proof",
    ...
    sections: [
      ...
      { heading: "验证问题", body: "高责任场景不能只看平均准确率，还要看最坏情况、权限隔离、操作日志、回滚与责任链。越接近现实决策和物理执行，人工复核与安全停机越不能省略。" },
    ],
  },
];
...
export const chapters: Chapter[] = [
  {
    ...
    watch: ["大会规模存在“3000 余项 / 4000 余款”不同口径，正文采用政府发布的前者。", "首发数量不等于可用数量，仍需逐项标注上市、量产或试点状态。"],
  },
  {
    slug: "compute",
    number: "03",
    ...
    sections: [
      ...
      { heading: "判断口径", body: "只在负载、精度和系统边界一致时比较参数。优先寻找真实模型训练 / 推理数据、客户、长期稳定运行和软件迁移成本，避免把单模块指标写成整机能力。", points: ["真实负载", "客户证据", "稳定运行", "生态成熟度"] },
    ],
    watch: ["多芯方案需要同时披露互联效率和软件适配成本。", "“全国产”应说明芯片、互联、服务器、存储和软件栈的具体边界。"],
  },
  {
    slug: "models-agents",
    number: "04",
    ...
    sections: [
      ...
      { heading: "横向框架", body: "评价智能体时依次检查任务拆解、工具范围、长任务记忆与恢复、云端 / 本地执行、数据隔离、权限审计、结果验证、价格与商业模式。", points: ["自主拆解", "工具调用", "失败恢复", "权限审计", "结果验证"] },
    ],
    watch: ["“能调用工具”不等于“能安全使用企业工具”。", "公开榜单与真实长任务之间仍有明显差距，应优先看端到端任务证据。"],
  },
  {
    slug: "embodied",
    number: "05",
    ...
    watch: ["要求披露随机任务比例、成功率和样本数。", "记录连续运行、人工接管、电池、维护、占地、节拍和回本周期。"],
  },
  {
    slug: "devices",
    number: "06",
    ...
    sections: [
      ...
      { heading: "成熟度", body: "统一按已上市现货、已预约 / 明确上市窗口、内测 / 邀请体验、展会首秀 / 概念机四层标注。只有同一成熟度层级的产品才适合直接比较功能和价格。", points: ["现货", "预约", "内测", "概念机"] },
    ],
    watch: ["贴身终端必须清楚提示何时录制、处理和上传。", "续航和跨应用权限往往比模型榜单更决定日常可用性。"],
  },
  {
    slug: "industry",
    number: "07",
    ...
    watch: ["行业效率数字需说明基线、样本和测量方法。", "高责任场景必须保留人工审批、日志、回滚和责任人。"],
  },
  {
    ...
    watch: ["奖项与媒体曝光只作为线索，不替代商业和技术验证。", "治理承诺应回到产品权限、审计、评测与事故响应机制。"],
  },
];
...
export const comparisonPages: { title: string; deck: string; matrices: ComparisonMatrix[] }[] = [
  {
    title: "跨厂商对比 / 系统与智能体",
    deck: "不做脱离口径的参数排名，只比较路线、验证边界和下一步需要补的证据。",
    matrices: [
      {
        title: "超节点与超集群",
